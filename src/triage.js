...
export async function triageItems(items) {
  ...
  const prompt =
    ...
    `most critical/impactful ones. Treat INCIDENTS and VULNERABILITIES as equal priorities — ` +
    `a major ransomware attack or breach is just as important as a critical CVE. ` +
    `Prioritize in this order:\n` +
    `1. Active security incidents: ransomware attacks, data breaches, nation-state intrusions, ` +
    `supply chain compromises, or major service outages caused by a cyberattack — especially against ` +
    `critical infrastructure, large enterprises, or widely-used services\n` +
    `2. Active 0-day exploitation or CISA KEV additions (always treat KEV additions as high priority — ` +
    `CISA adding a CVE to KEV means active exploitation is confirmed regardless of the CVE's age)\n` +
    `3. ZDI, MSRC, Red Hat, or OSV advisories for RCE / privilege escalation in widely-deployed software\n` +
    `4. GitHub Advisories (GHSA) for supply chain / open source RCE\n` +
    `5. Threat intel from Talos, Unit 42, Mandiant, Securelist — novel campaigns, malware, or TTPs\n` +
    `6. Web app RCE, SQLi, auth bypass, XSS in widely-used software\n` +
    `7. Critical/High CVEs (CVSS ≥ 8.5) in common infrastructure or cloud services\n\n` +
    ...
    `Prefer freshly assigned CVEs, Patch Tuesday releases, recently published advisories, and ` +
    `breaking incident reports.\n\n` +
